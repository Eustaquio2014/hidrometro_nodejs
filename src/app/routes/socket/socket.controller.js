...
const gravarFluxo = async (content) => {
  ...
  if (parseFloat(novoFluxo) && novoMac) {
    const dispositivoData = await Prisma.dispositivo.findUnique({ where: { macId: novoMac } });
    ...
    if (dispositivoData?.idUsuario) {
      ...
      await Prisma.Hidrometro.create({
        ...
        .then((result) => {
          console.log('Sucesso! gravarFluxo!', result);

          return true;
        })
        .catch((error) => {
          console.log(error.message);
          console.log(`Erro ao salvar gravarFluxo! ${capitalizeFirstLetter(error.meta?.target[0])}`);
