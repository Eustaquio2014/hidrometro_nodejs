...
router.post('/consumo', async (req, res) => {
  const dispositivosIds = req.session.userSessionData.dispositivos.map((dispositivo) => dispositivo.macId);

  const { startDate, endDate, deviceId, groupBy } = req.body;
  const selectedDeviceMac = dispositivosIds.find((id) => String(id) === deviceId);

  let responseData = [];

  const [dataRecente] = await getLastHidrometroDataByMacId(selectedDeviceMac || dispositivosIds[0]);
